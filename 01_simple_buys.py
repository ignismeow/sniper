...
def sell_token(token_address, amount_in_wei, nonce):
    token_address = to_checksum(token_address)
    # Approve the Uniswap Router to spend the token from your wallet
    token_contract = provider.eth.contract(address=token_address, abi=blockchain['ERC20Abi'])
    ...
    signed_approve_txn = provider.eth.account.sign_transaction(approve_txn, private_key)
    approve_txn_hash = provider.eth.send_raw_transaction(signed_approve_txn.raw_transaction)
    print("Approve transaction sent for sell with hash:", approve_txn_hash.hex())
    
    # Wait for approval to be mined
    provider.eth.wait_for_transaction_receipt(approve_txn_hash)

    # Perform the swap to sell the token
    weth_address = to_checksum(blockchain['WETHAddress'])
    sell_txn = router_contract.functions.exactInputSingle({
        'tokenIn': token_address,               # Token address to sell
        'tokenOut': weth_address,               # WETH address
        'fee': 3000,                           # Default fee tier
        'recipient': wallet_address,            # Your wallet address
        'deadline': int(provider.eth.get_block('latest')['timestamp']) + 60 * 10,  # 10 minutes from now
        'amountIn': amount_in_wei,              # Amount of token being sold (in wei)
        'amountOutMinimum': 0,                  # Minimum amount of WETH (adjust for slippage)
        'sqrtPriceLimitX96': 0                  # Set price limit to 0 (no limit)
    }).build_transaction({
        'from': wallet_address,
        'nonce': nonce + 1,  # Increment nonce for the new transaction
        'gas': 300000,
        'gasPrice': provider.to_wei('45', 'gwei')  # Increased gas price
    })

    # Sign and send the sell transaction
    signed_sell_txn = provider.eth.account.sign_transaction(sell_txn, private_key)
    sell_txn_hash = provider.eth.send_raw_transaction(signed_sell_txn.raw_transaction)
    print("Sell transaction sent with hash:", sell_txn_hash.hex())

    # Wait for the transaction to be mined
    receipt = provider.eth.wait_for_transaction_receipt(sell_txn_hash)
    print("Sell transaction mined in block:", receipt.blockNumber)

# Function to swap WETH to the selected token from the CSV
def swap_tokens_from_csv(num_trades):
    ...
    with open(SNIPE_LIST_FILE, newline='') as csvfile:
        ...
        for _ in range(num_trades):
            for row in reader:
                ...
                break  # Break after one trade per pair

# Main function to get user input and execute trades
def main():
    num_trades = int(input("Enter the number of trades to perform: "))
    swap_tokens_from_csv(num_trades)
